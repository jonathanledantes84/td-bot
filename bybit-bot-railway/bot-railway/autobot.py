# -*- coding: utf-8 -*-
"""
AUTOBOT.PY - SuperTrend Bot para Bybit (version Railway)
"""
...
def telegram(msg):
    ...
    try:
        requests.post(f"https://api.telegram.org/bot{config.TELEGRAM_TOKEN}/sendMessage",
                      json={"chat_id": config.TELEGRAM_CHAT_ID, "text": msg}, timeout=5)
    except Exception as e:
        log(f"Telegram error: {e}", "WARN")

def dentro_de_horario():
    return config.TRADING_HOUR_START <= datetime.now().hour < config.TRADING_HOUR_END
...
def check_stop_loss(symbol):
    ...
    if not pos or config.STOP_LOSS_PCT <= 0: return False
...
def check_daily_limit():
    if config.DAILY_LOSS_LIMIT_PCT <= 0 or not state["saldo_inicio_dia"]: return False
...
def run_symbol(sym_cfg, last_trends):
    ...
    if last is not None:
        ...
        else: log(f"[{symbol}] Sin cambio, esperando...")
    else:
        log(f"[{symbol}] Primera lectura...")
...
def run():
    ...
    log(f"Horario: {config.TRADING_HOUR_START}h-{config.TRADING_HOUR_END}h | SL: {config.STOP_LOSS_PCT}% | Limite diario: {config.DAILY_LOSS_LIMIT_PCT}%")
    print("-" * 60)
    ...
    while True:
        try:
            ...
            hilos = [threading.Thread(target=run_symbol, args=(s, last_trends)) for s in config.SYMBOLS]
