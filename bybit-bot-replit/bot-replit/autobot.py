# -*- coding: utf-8 -*-
"""
AUTOBOT.PY - SuperTrend Bot para Bybit (version Replit)
Incluye: Multi-par, Stop Loss, Limite diario, Horario, Telegram, Log, Auto-reinicio, Keep-alive
"""
...
def run_symbol(sym_cfg, last_trends):
    ...
    if state["detenido"]:
        log(f"[{symbol}] Detenido por limite diario", "WARN"); return
    if not dentro_de_horario():
        log(f"[{symbol}] Fuera de horario ({config.TRADING_HOUR_START}h-{config.TRADING_HOUR_END}h)"); return
...
def run():
    ...
    while True:
        try:
            ...
            log("Bot detenido."); telegram("Bot detenido manualmente."); break
        except Exception as e:
            log(f"Error: {e}", "ERROR"); time.sleep(CHECK_INTERVAL)

# ── ARRANQUE ─────────────────────────────────────────────
if __name__ == "__main__":
    keep_alive()  # inicia servidor para UptimeRobot
